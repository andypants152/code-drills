...
function review() {
    ...
    connection.query(
        ...
        function (err, res) {
            ...
            var table = new Table({
                head: ['Show Name', 'Num of Seasons', 'Genre', 'Rating']
                });
            
                res.forEach(elem => {
                    table.push([elem.show_name, elem.num_seasons, elem.genre, elem.rating]);
                });
            table.push();
...
function updateRating(list) {
            ...
            inquirer.prompt([{
                ...
                var queryString = "UPDATE nominees SET ? WHERE ?"
                    connection.query(queryString, [{rating: answers.rating},{show_name: answers.nominee}],
